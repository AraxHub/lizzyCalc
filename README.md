...
В репозитории есть `.gitignore` с правилом для файла **`.env`**. Его не коммитят, потому что там могут лежать пароли и локальные настройки — они не должны попадать в общий репо. Шаблон настроек хранится в **`.env.example`** (без секретов): его коммитят, а каждый разработчик копирует в `.env` и подставляет свои значения. В проде переменные задают через окружение хостинга, без файла `.env`.
