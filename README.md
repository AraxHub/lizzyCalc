...
**Требования:** для поднятия окружения (БД, Redis, Kafka и т.д. по веткам) нужна установка **Docker**. Рекомендуется **Docker Desktop** (Windows/macOS). Полезно понимание **SQL** на базовом уровне (SELECT, INSERT, простые запросы).
