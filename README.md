...
| 11 | `feature-grpc-api` | gRPC-сервер на отдельном порту (например 9090): тот же use case (Calculate, History), второй транспорт рядом с REST. Proto, кодоген, вызовы из клиента. |
| — | `main` | Текущая основная ветка. |

Кратко маршрут: логика и «БД» в памяти → HTTP → объединение → Gin и версионирование → реальная БД → env → архитектура → MVP → фронт → кэш (Redis) → gRPC.
...
- [ ] **feature-makefile** — Makefile с целями для сборки, запуска, тестов, Docker (build, run, test, docker-up, docker-down и т.д.).
